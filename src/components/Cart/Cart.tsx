...
import { useGetOrder, ShopState } from '../../customHooks/FetchData'
...
export const Cart = () => {
    ...
    useEffect(()=> {
        // we will use theonValue() to listen for changes in our cart
        ...
        // cleanup the listener when it detatches
        return()  =>{
    ...
    const updateQuantity = async (id: string, operation: string) => {
        ...
        const dataIndex = currentCart.findIndex((cart) => cart.id == id) // looping through our list until it finds a match and returns its index
    ...
    const updateCart = async ( cartItem: ShopState ) => {
        ...
        // use update() function to update a specific item
        update(itemRef, {
    ...
    return (
        <Box sx={shopStyles.main} >
            ...
            <Stack direction = 'column' sx={shopStyles.main}>
                <Stack direction = 'row' sx={{alignItems: 'center', marginTop: '100px', marginLeft: '100px'}}>
                    <Typography
                        variant = 'h4'
                        sx = {{marginRight: '20px'}}
                    >
